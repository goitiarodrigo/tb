...
export const filesServices = {
    ...
    getFiles: async () => {
        try {
            const response = await axios.get(`${EXTERNAL_BASE_URL}/files`, headers);
    ...
    processCsvFile: async (files) => {
        try {
            const promises = files.map(async (file) => {
                try {
                    const response = await axios.get(`${EXTERNAL_BASE_URL}/file/${file}`, headers);
                    if (response.status !== 200) {
                        throw new Error(`Error to get file: ${response.data.message}`);
                    }
